...
const getProcessingTimeInMS = (time: [number, number]) : string => {
    return `${(time[0] * 1000 + time[1] * 1e-6).toFixed(2)}ms`;
...
export default function logger(req: Request, res: Response, next: NextFunction) {
    ...
    console.log(`${id}${timestampText} ${method}:${url} ${startText}`);
    ...
    res.once('finish', () => {
        ...
        console.log(`${id} ${timestampText} ${method}:${url} ${res.statusCode} ${endText}`);
    });


    // execute the next middleware/event handler
    next();
};